...
def calc_dept(inputs, hr_data, c12_inputs, section_totals, calc_periods, rev_lgh, rev_c12):
    ...
    for p in calc_periods:
        ...
        inc = -(pp + pme) * (df + dsf)
        dec = -(pp + cue) * (pfa + psa)
        store('increase_accrued_participation', '', p, inc)
        store('decrease_accrued_participation', '', p, dec)
        # cumm_accrued_participation = increase + decrease + prior, seeded from the opening
        # column. This value crosses to c12_oc (total_c12_acc_rec), so it is an OUTPUT and
        # is produced here (was previously read from the dept sheet's mislabelled `i` row).
        store('cumm_accrued_participation', '', p, inc + dec + prior('cumm_accrued_participation', p))
...
# The pieces are split so the runner can thread results between modules IN MEMORY:
#   load()        -> open the workbook once (formulas + data_only) and read c12_inputs
#   compute_dept()-> PURE: read one department's inputs and return its results dict
#   diff_o_rows() -> compare a results dict against the sheet's `o` cells
#   calibrate()   -> load + compute + diff + print (isolated, single-module check)

def load(excel_file=EXCEL_FILE):
    """Open the workbook once. Returns (wb, wb_data, c12_inputs, proj_from, proj_to)."""
    wb      = openpyxl.load_workbook(excel_file)
    wb_data = openpyxl.load_workbook(excel_file, data_only=True)
    ci = read_c12_inputs(wb)
    return wb, wb_data, ci, ci.get('projections_from'), ci.get('projections_to')

def compute_dept(wb_data, ci, sheet, hr_sheet, pf, pt):
    """Read a department's inputs/HR/section-totals and return its results dict.
    Returns (results, period_cols, rev_lgh, rev_c12, dupes)."""
    ws = wb_data[sheet]
    ...
    results = calc_dept(inputs, hr, ci, sect, list(period_cols), rev_lgh, rev_c12)
    return results, period_cols, rev_lgh, rev_c12, inputs.get('_dupes', [])

def diff_o_rows(wsd, results, period_cols, tol=0.5):
    """Compare Python results against every `o` cell on the sheet. Returns a list of
    (field, index, period, excel, python, diff). Rows Python doesn't produce are skipped."""
    diffs = []
    for r in range(1, wsd.max_row + 1):
        if wsd.cell(r, 2).value != 'o':
            continue
        f = wsd.cell(r, 3).value; i = wsd.cell(r, 4).value or ''
        if not f or (f, i) not in results:
            continue
        for p, c in period_cols.items():
            e = wsd.cell(r, c).value; e = e if isinstance(e, (int, float)) else 0.0
...
def calibrate(sheet, hr_sheet, excel_file=EXCEL_FILE, tol=0.5, verbose=True):
    """Isolated single-department check: compute vs the sheet's own Excel formulas."""
    wb, wbd, ci, pf, pt = load(excel_file)
    results, pcs, rev_lgh, rev_c12, dupes = compute_dept(wbd, ci, sheet, hr_sheet, pf, pt)
    diffs = diff_o_rows(wbd[sheet], results, pcs, tol)
    if verbose:
        print(f"[{sheet}] periods {list(pcs)} | revenue fields: {rev_lgh}, {rev_c12}")
        if dupes:
